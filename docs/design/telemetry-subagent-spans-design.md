...
| Agent identity ALS                 | `packages/core/src/subagents/agent-context.ts:32` `runWithAgentContext(agentId, ...)`                                                                                                            | Already carries `agentId` + `subagentName`; we extend it with `depth` |
